        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    callbacks: {
                        label: function(context) {
                            return `${context.raw.toFixed(1)}% chance of ${context.label}`;
                        }
                    },
                    padding: 10,
                    backgroundColor: 'rgba(255, 255, 255, 0.9)',
...
// Create VIP feature importance chart
function createVIPFeatureImportanceChart(featureImportance) {
    // Destroy previous chart if it exists
    ...
    gradient.addColorStop(0, 'rgba(52, 168, 83, 0.9)');
    gradient.addColorStop(1, 'rgba(52, 168, 83, 0.6)');
    
    modelConfidenceChart = new Chart(ctx, {
        type: 'bar',
        data: {
            labels: topFeatures.map(([name]) => name),
            datasets: [{
                label: 'Importance %',
        ...
        options: {
            ...
            plugins: {
                ...
                title: {
                    display: true,
                    text: 'VIP Feature Importance Analysis',
                    font: {
                        size: 16,
                        weight: 'bold'
                    },
                ...
                tooltip: {
                    callbacks: {
                        label: function(context) {
                            return `${context.raw.toFixed(1)}% influence on prediction`;
                        }
                    },
                    padding: 10,
                    backgroundColor: 'rgba(255, 255, 255, 0.9)',
    ...
    // Add VIP badge to chart
    const chartContainer = document.querySelector('#model-confidence-chart').closest('.chart-card');
...
// Create VIP performance trend chart
function createVIPPerformanceTrendChart() {
    ...
    const team1Matches = [...matchData.h2h, ...matchData.team1].sort((a, b) => a.timestamp - b.timestamp);
    const team2Matches = [...matchData.h2h, ...matchData.team2].sort((a, b) => a.timestamp - b.timestamp);
    
    // Calculate moving averages
    const calculateMovingAverage = (matches, teamNumber, window = 3) => {
        const results = [];
        const scores = [];
        ...
        matches.forEach((match, index) => {
            ...
            if (match.category === 'h2h') {
                teamScore = teamNumber === 1 ? match.team1Score : match.team2Score;
                opponentScore = teamNumber === 1 ? match.team2Score : match.team1Score;
            } else {
            ...
            // Calculate moving average for the last 'window' matches
            if (index >= window - 1) {
                const recentScores = scores.slice(index - window + 1, index + 1);
                const average = recentScores.reduce((sum, score) => sum + score, 0) / window;
                results.push({
                    x: index + 1,
                    y: average,
    ...
    const team1Trend = calculateMovingAverage(team1Matches, 1);
    const team2Trend = calculateMovingAverage(team2Matches, 2);
    ...
    performanceTrendChart = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                ...
                title: {
                    display: true,
                    text: 'VIP Performance Trend Analysis (3-Match Moving Average)',
                    font: {
                        size: 16,
                        weight: 'bold'
                    },
                ...
                tooltip: {
                    callbacks: {
                        title: function(context) {
                            return `Match ${context[0].parsed.x}`;
                        },
                        label: function(context) {
                            return `${context.dataset.label}: ${context.parsed.y.toFixed(2)} goals/match`;
                        }
                    },
                    padding: 12,
                    backgroundColor: 'rgba(255, 255, 255, 0.9)',
            ...
            scales: {
                ...
                y: {
                    ...
                    title: {
                        display: true,
                        text: 'Goals per Match (Moving Average)',
                        font: {
                            size: 12,
                            weight: '500'
                        }
    ...
    // Add VIP badge to chart
    const chartContainer = document.querySelector('#performance-trend-chart').closest('.chart-card');
    if (chartContainer && !chartContainer.querySelector('.vip-chart-badge')) {
        const vipBadge = document.createElement('div');
        vipBadge.className = 'vip-chart-badge';
        vipBadge.innerHTML = 'VIP';
...
function createVIPBettingEdgeChart(bettingAnalysis) {
    ...
    if (!document.getElementById('betting-edge-chart')) {
        ...
        newChartCard.className = 'chart-card';
        newChartCard.innerHTML = `
            <h3>VIP Betting Edge Analysis</h3>
            <div class="chart-container">
    ...
    // Prepare data showing the strength of different betting options
    ...
    // Add win-draw-win options
    if (bettingAnalysis.winDrawWin.team1.probability > 0.4) {
        bettingOptions.push({
            label: `${team1Name} Win`,
            probability: bettingAnalysis.winDrawWin.team1.probability,
            type: 'team1'
        });
    }
    
    if (bettingAnalysis.winDrawWin.team2.probability > 0.4) {
        bettingOptions.push({
            label: `${team2Name} Win`,
            probability: bettingAnalysis.winDrawWin.team2.probability,
            type: 'team2'
        });
    }
    
    if (bettingAnalysis.winDrawWin.draw.probability > 0.3) {
        bettingOptions.push({
            label: `Draw`,
            probability: bettingAnalysis.winDrawWin.draw.probability,
            type: 'draw'
        });
    }
    
    // Add BTTS options
    if (bettingAnalysis.bothTeamsToScore.yes.probability > 0.52) {
        bettingOptions.push({
            label: `BTTS Yes`,
            probability: bettingAnalysis.bothTeamsToScore.yes.probability,
            type: 'btts'
        });
    }
    
    if (bettingAnalysis.bothTeamsToScore.no.probability > 0.52) {
        bettingOptions.push({
            label: `BTTS No`,
            probability: bettingAnalysis.bothTeamsToScore.no.probability,
            type: 'btts'
        });
    }
    
    // Sort by probability and take top 10
    bettingOptions.sort((a, b) => b.probability - a.probability);
    const topOptions = bettingOptions.slice(0, 10);
    
    if (topOptions.length === 0) {
        // No strong betting edges found
        const ctx = document.getElementById('betting-edge-chart').getContext('2d');
        bettingEdgeChart = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: ['No Strong Edges Found'],
                datasets: [{
                    label: 'Edge Strength',
                    data: [50],
            ...
            options: {
                ...
                plugins: {
                    ...
                    title: {
                        display: true,
                        text: 'VIP Betting Edge Analysis - No Strong Edges Detected',
                        font: {
                            size: 16,
                            weight: 'bold'
                        }
    ...
    // Create color mapping for different bet types
    const colorMap = {
        'over': 'rgba(255, 193, 7, 0.8)',
        'under': 'rgba(13, 110, 253, 0.8)',
        'team1': 'rgba(66, 133, 244, 0.8)',
        'team2': 'rgba(234, 67, 53, 0.8)',
        'draw': 'rgba(95, 99, 104, 0.8)',
        'btts': 'rgba(52, 168, 83, 0.8)'
    };
    
    const borderColorMap = {
        'over': 'rgba(255, 193, 7, 1)',
        'under': 'rgba(13, 110, 253, 1)',
        'team1': 'rgba(66, 133, 244, 1)',
        'team2': 'rgba(234, 67, 53, 1)',
        'draw': 'rgba(95, 99, 104, 1)',
        'btts': 'rgba(52, 168, 83, 1)'
    };
    ...
    bettingEdgeChart = new Chart(ctx, {
        type: 'bar',
        data: {
            labels: topOptions.map(opt => opt.label),
            datasets: [{
                label: 'Probability (%)',
                data: topOptions.map(opt => opt.probability * 100),
                backgroundColor: topOptions.map(opt => colorMap[opt.type]),
                borderColor: topOptions.map(opt => borderColorMap[opt.type]),
                borderWidth: 2,
                borderRadius: 6
            }]
        },
        options: {
            ...
            plugins: {
                ...
                title: {
                    display: true,
                    text: 'VIP Betting Edge Analysis - Top Opportunities',
                    font: {
                        size: 16,
                        weight: 'bold'
                    },
                ...
                tooltip: {
                    callbacks: {
                        label: function(context) {
                            ...
                            const edge = Math.max(0, probability - 50);
                            return [
                                `Probability: ${probability.toFixed(1)}%`,
                                `Edge: ${edge.toFixed(1)}%`,
                                `Strength: ${probability > 65 ? 'Strong' : probability > 55 ? 'Moderate' : 'Weak'}`
                            ];
                        }
                    },
                    padding: 12,
...
// Ensure prediction consistency between probabilities and margin
function ensurePredictionConsistency(probabilities, margin) {
    // Adjust margin based on probabilities if there's inconsistency
    const probMargin = probabilities.team1WinProb - probabilities.team2WinProb;
    
    // If probability margin and score margin have opposite signs, adjust
    if (Math.sign(probMargin) !== Math.sign(margin) && Math.abs(margin) > 0.5) {
        // Adjust margin to match probability direction
        margin = Math.sign(probMargin) * Math.abs(margin) * 0.7;
    }
    
    // Ensure probabilities sum to 100%
    const total = probabilities.team1WinProb + probabilities.team2WinProb + probabilities.drawProb;
    if (Math.abs(total - 100) > 0.1) {
        probabilities.team1WinProb = (probabilities.team1WinProb / total) * 100;
        probabilities.team2WinProb = (probabilities.team2WinProb / total) * 100;
        probabilities.drawProb = (probabilities.drawProb / total) * 100;
    }
    
    return [probabilities, margin];
}

// Prepare standard match features (used by VIP and standard analysis)
...
function calculateAdvancedStats(basicStats) {
    ...
    // Match importance performance
    const team1MatchImportancePerformance = calculateMatchImportancePerformance('team1');
    const team2MatchImportancePerformance = calculateMatchImportancePerformance('team2');
...
// Calculate home advantage (dummy function - in real implementation would use historical data)
...
// Calculate match importance performance (dummy function)
function calculateMatchImportancePerformance(teamKey) {
    // For now, return a default performance in important matches
    // In a real implementation, this would analyze performance in high-stakes games
...
// Standard analysis function (fallback when VIP analysis fails)
function performAnalysis() {
    try {
        ...
        // Calculate simple betting recommendations
        const bettingRecommendations = calculateStandardBettingRecommendations(results);
        ...
        updateScorePrediction(
            Math.round((results.projectedTotal / 2) + (results.projectedMargin / 2)),
            Math.round((results.projectedTotal / 2) - (results.projectedMargin / 2)),
            results.projectedTotal,
            totalLine
        );
        updateBettingRecommendation(bettingRecommendations);
        updateAnalysisExplanation(results, features);
...
function calculateProjectedTotal(features) {
    ...
    if (features.dataQuality.h2hMatches >= 2) {
        const h2hAvgTotal = matchData.h2h.reduce((sum, match) => sum + match.totalScore, 0) / 
                           features.dataQuality.h2hMatches;
        const h2hWeight = Math.min(0.4, features.dataQuality.h2hMatches * 0.08);
        baseTotal = baseTotal * (1 - h2hWeight) + h2hAvgTotal * h2hWeight;
    }
    
    // Adjust for form
    baseTotal += ((advancedStats.team1RecentForm + advancedStats.team2RecentForm) - 1) * 0.4;
    
    // Adjust for attack/defense
    baseTotal += (advancedStats.team1AttackStrength + advancedStats.team2AttackStrength - 2) * 0.4;
    baseTotal -= ((1 - advancedStats.team1DefenseStrength) + (1 - advancedStats.team2DefenseStrength)) * 0.4;
    
    // Match importance adjustment
    if (basicStats.matchImportance < 1) {
        baseTotal += (1 - basicStats.matchImportance) * 0.5;
    } else if (basicStats.matchImportance > 1.3) {
        baseTotal -= (basicStats.matchImportance - 1.3) * 0.3;
    }
    
    return Math.max(0.5, baseTotal);
}

// Calculate projected margin (standard version)
function calculateProjectedMargin(features) {
    ...
    // Adjust for form
    baseMargin += (advancedStats.team1RecentForm - advancedStats.team2RecentForm) * 0.6;
    
    // Adjust for strength
    baseMargin += ((advancedStats.team1AttackStrength - advancedStats.team2DefenseStrength) - 
                  (advancedStats.team2AttackStrength - advancedStats.team1DefenseStrength)) * 0.4;
    
    // Location adjustment
    if (basicStats.locationFactor !== 0) {
        baseMargin += basicStats.locationFactor * 0.3;
    }
    
    return baseMargin;
}

// Calculate standard betting recommendations
function calculateStandardBettingRecommendations(results) {
    const recommendations = [];
    
    // Win-Draw-Win recommendations
    const maxProb = Math.max(
        results.probabilities.team1WinProb,
        results.probabilities.team2WinProb,
        results.probabilities.drawProb
    );
    
    if (maxProb > 55) {
        if (maxProb === results.probabilities.team1WinProb) {
            recommendations.push({
                type: '1X2',
                selection: `${team1Name} to Win`,
                probability: maxProb,
                strength: maxProb > 70 ? 'Strong' : 'Moderate'
            });
        } else if (maxProb === results.probabilities.team2WinProb) {
            recommendations.push({
                type: '1X2',
                selection: `${team2Name} to Win`,
                probability: maxProb,
    ...
    if (totalLine > 0) {
        // Calculate probability of over/under based on projected total
        const lambda = results.projectedTotal / 2; // Assume Poisson distribution
        
        let overProb = 0;
        for (let goals = Math.ceil(totalLine); goals <= 10; goals++) {
            overProb += Math.pow(lambda, goals) * Math.exp(-lambda) / factorial(goals);
        }
        overProb = overProb * 100;
        ...
        if (overProb > 60) {
            recommendations.push({
                ...
                probability: overProb,
                strength: overProb > 75 ? 'Strong' : 'Moderate'
            });
        } else if (underProb > 60) {
            recommendations.push({
                type: 'Over/Under',
                selection: `Under ${totalLine}`,
                probability: underProb,
                strength: underProb > 75 ? 'Strong' : 'Moderate'
            });
        }
    }
...
function calculateStandardFeatureImportance(features) {
    const importance = {
        'Head-to-Head History': features.dataQuality.h2hMatches >= MIN_H2H_MATCHES ? 
            Math.abs(features.basicStats.h2hAdvantage) * 100 : 25,
        ...
        'Home Advantage': features.basicStats.locationFactor !== 0 ?
            Math.abs(features.basicStats.locationFactor) * 50 : 15,
        
        'Team Momentum': 
            Math.abs(features.advancedStats.team1MomentumIndex - features.advancedStats.team2MomentumIndex) * 60
    };
...
// Update betting recommendation UI (standard version)
function updateBettingRecommendation(recommendations) {
...
// Update analysis explanation UI (standard version)
function updateAnalysisExplanation(results, features) {
    let winnerName, winnerProb;
    ...
    const explanationHTML = `
        ...
        <p><strong>${winnerName}</strong> has a ${winnerProb.toFixed(1)}% probability of winning this match with a projected score of <strong>${team1Score}-${team2Score}</strong>.</p>
        
        <h4>Key Factors:</h4>
        <ul>
            <li><strong>Data Quality:</strong> Analysis based on ${features.dataQuality.totalMatches} total matches (${features.dataQuality.h2hMatches} head-to-head)</li>
            <li><strong>Team Form:</strong> Recent performance and momentum analysis</li>
            <li><strong>Matchup History:</strong> Head-to-head performance patterns</li>
            <li><strong>Home/Away:</strong> Location-based performance adjustments</li>
        </ul>
        
        <p><em>Predictions are based on statistical analysis of historical performance data and should be used as guidance only.</em></p>
    `;
    
    document.getElementById('analysis-explanation').innerHTML = explanationHTML;
}

// Create standard win probability chart
...
// Create standard feature importance chart
function createFeatureImportanceChart(featureImportance) {
    ...
    const topFeatures = Object.entries(featureImportance).slice(0, 6);
...
// Create standard performance trend chart
    ...
    performanceTrendChart = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                ...
                title: {
                    display: true,
                    text: 'Performance Trends (3-Match Moving Average)'
                }
            },
            scales: {
                x: {
